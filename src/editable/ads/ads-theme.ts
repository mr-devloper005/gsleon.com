...
export const adSkin: AdSkin = {
  ...
  shadow: 'none',
  background: '#ffffff',
  labelClassName: 'bg-[#fe4804] text-white',
}

// Optional per-slot overrides — adjust only where you need to.
export const adSkinBySlot: Partial<Record<string, AdSkin>> = {
  sidebar: { radius: '0px', shadow: 'none', border: '1px solid #e6e6e6' },
  popup: { radius: '0px' },
  header: { radius: '0px', background: '#f4f4f4' },
  rail: { radius: '0px' },
  feature: { radius: '0px' },
  interstitial: { radius: '0px', shadow: '0 20px 60px rgba(0,0,0,0.5)' },
  anchor: { radius: '0px', shadow: '0 6px 24px rgba(0,0,0,0.18)' },
}
