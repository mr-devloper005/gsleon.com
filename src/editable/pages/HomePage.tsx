...
import {
  ...
  EditableFeaturedResources,
  EditableHomeCta,
  EditableHomeFaq,
  EditableHomeHero,
  EditableHomeProof,
  EditableResourceFeatures,
...
export default async function HomePage() {
  ...
  const visibleFeed = taskFeed.filter(({ task }) => !isUiHiddenTask(task.key))
  const primaryPosts = uniquePosts(visibleFeed.find(({ task }) => task.key === primaryTask)?.posts || visibleFeed.flatMap(({ posts }) => posts)).slice(0, 24)
  ...
  return (
    <EditableSiteShell>
      <main>
        ...
        <div className="mx-auto max-w-[var(--editable-container)] px-5 py-8 sm:px-8 lg:px-16">
          <Ads slot="header" showLabel eager className="mx-auto w-full" />
        </div>
        <EditableCollectionsMarquee primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} timeSections={timeSections} />
        <EditableResourceFeatures primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} timeSections={timeSections} />
        <EditableFeaturedResources primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} timeSections={timeSections} />
        <EditableTimeCollections primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} timeSections={timeSections} />
        <EditableHomeProof posts={primaryPosts} />
        <EditableHomeFaq />
        <EditableHomeCta />
      </main>
    </EditableSiteShell>
